...
import { setCount } from "../slices/mesuSlice";
...
function ToDoList() {
  ...
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        const data = await storage.load<ToDo[]>("ToDoList");
  ...
  const loadUnCompletedToDos = async () => {
    const unCompletedToDosCount = toDos.filter((x) => x.completed === false).length

    dispatch(setCount(unCompletedToDosCount))
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title}>{new Date().toLocaleDateString("uk-UA")}</Text>
      <FlatList
        data={toDos}
        renderItem={({ item }) => (
